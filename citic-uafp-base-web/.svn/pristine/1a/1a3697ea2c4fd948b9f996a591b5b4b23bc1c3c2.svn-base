...
<div class="page-content"  id="page-content">
	 <fm:form class="form-horizontal" commandName="cgb_risk_case"  action="check_list"  method="POST">	
					...
					<div class="widget-box">
						<div class="widget-header widget-header-flat">
							...
							<div class="widget-toolbar">
                                <a href="#" onclick="do_tj('<c:url value="/risk_case/choose_save?bzw=2" />','确认审核通过?');">
							...
							<div class="row">		
										...
										<label class="control-label col-xs-4 col-sm-4 no-padding-right">部门/支行：</label>
									    <div class=" col-xs-8 col-sm-8 input-group">
									           <fm:input type="hidden"  path="suborgankey"/>
								             	...
								             	<a href="#"  onclick="selectTreeByName('suborgankey_disp','selectOrgan');"   title="选择交易机构">
												...
												<div class="form-group col-xs-12 col-sm-6">
													<label class="control-label col-xs-12 col-sm-4 no-padding-right" >发生日期：</label>
							
													<div class="input-group col-xs-12 col-sm-8">
														<div class=" input-group col-xs-12 col-sm-9">
															<fm:input class="form-control date-picker" path="happendate_q" type="text" data-date-format="yyyy-mm-dd" />
                                                   <span class="input-group-addon">
                                                  <i class="fa fa-arrows-h"></i>
                                                    </span>
															<fm:input class="form-control date-picker" path="happendate_z" type="text" data-date-format="yyyy-mm-dd" />
                                                   <span class="input-group-addon">
									...
									<div class="row">
										<div class="col-xs-12">		
												<div class="form-group col-xs-12 col-sm-6">
													<label class="control-label col-xs-12 col-sm-4 no-padding-right" >涉案户名：</label>
													<div class="input-group col-xs-12 col-sm-8">
								...
								<div class="row">		
										<div class="col-xs-12">		
												...
												<div class="form-group col-xs-12 col-sm-6">
													<label class="control-label col-xs-12 col-sm-4 no-padding-right">状态：</label>
													<div class="input-group col-xs-12 col-sm-8">
													<fm:select path="status_cd" items="${status_cdMap}"></fm:select>&nbsp; 
														<button class="btn btn-info btn-mini" type="button" id="submitForm" onclick="_submit('check_list?isNewSearch=1')" >
															<i class="glyphicon glyphicon-search"></i>
															查询
														</button>
													</div>
						...
						<div class="widget-main no-padding">
							<table class="table table-striped table-bordered table-hover"
								...
								<c:forEach items="${cgb_risk_caseList}" var="cases" varStatus="s">
										...
										<c:choose>
										   <c:when test="${cases.status_cd=='1'}">
										       <td class="align-middle" nowrap="nowrap">
													<div class="checkbox">
														<label> <input type="checkbox" class="ace"
															name="keys" value="${cases.caseid}"/> <span class="lbl"></span>
														</label>
													</div>
												</td>
								    <td class="align-middle" nowrap="nowrap"><a class="green"
																href="#" onclick="do_back('<c:url value="/risk_case/back?caseid=${cases.caseid}&bzw=2"/>')"
																title="退回"><i class="ace-icon fa fa-arrow-circle-left red bigger-110"></i>
															</a>
                                    </td>
										   </c:when>
										   <c:otherwise>
								    ...
								    <td  class="align-middle" nowrap="nowrap" ><a href="#" onclick="_tips(this,'${cases.casedesc}')">${cases.casedesc_disp}</a></td>
								    ...
								    <td class="align-middle" nowrap="nowrap" ><span class="label label-info arrowed-in arrowed-in-right">${cases.status_cd_disp}</span></td>
								</tr>
									</c:forEach>
								</tbody>
							</table>
		...
		<script src="<c:url value='/resources/js/lib/artDialog/6.0.4/dist/dialog-plus.js' />"> </script>
		<script src="<c:url value='/resources/js/agg/agg.js' />"> </script>
		...
		function do_tj(url,title) {
         ...
         function do_back(url){
        	 $("#url").val(url);
        	 agg.dialogConfirm("确认退回?",okDo,closeDo);
         }
