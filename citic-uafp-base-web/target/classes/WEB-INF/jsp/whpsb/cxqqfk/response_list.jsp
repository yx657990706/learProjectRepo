...
<!-- PAGE CONTENT BEGINS -->
  <fm:form class="form-horizontal"  commandName="br51_cxqq_back"  action="response_list"  method="POST">
   ...
   <div class="widget-body">
        <div class="widget-main">
                ...
                <div class="row">
                    <div class="col-xs-12">
                     <div class="form-group col-xs-12 col-sm-6">
                            <label class="control-label col-xs-12 col-sm-4 no-padding-right" >核心处理状态：</label>
                            <div class="input-group col-xs-12 col-sm-8">
                                <fm:select path="status" class=" input-medium" items="${statusMap}"/>
                            </div>
                        </div>
                        <div class="form-group col-xs-12 col-sm-6">
                            <label class="control-label col-xs-12 col-sm-4 no-padding-right" >处理结果：</label>
                            <div class="input-group col-xs-12 col-sm-8">
    ...
    <div class="widget-body">
        <div class="widget-main no-padding">
            <table class="table table-striped table-bordered table-hover"  id="gs">
                ...
                <tbody>
				<c:forEach var="cxqqList"  items="${br51_cxqq_backList}" >
                <tr class="text-center">
                    <td nowrap><a href="response_main?bdhm=${cxqqList.bdhm }&ah=${cxqqList.ah }&qrymode=${cxqqList.qrymode }&msgseq=${cxqqList.msgseq }&party_class_cd=${cxqqList.party_class_cd }"> ${cxqqList.bdhm }</a></td>
                     <td nowrap>${cxqqList.ah }</td>
                    <td nowrap>${cxqqList.msgseq }</td>
                    <td nowrap>
                    <c:if test="${cxqqList.cljg=='0'}">
                    <span class="label label-success arrowed-in arrowed-in-right">${cxqqList.cljg_disp }</span>
                    </c:if>
                    <c:if test="${cxqqList.cljg=='1'}">
                   <span class="label label-danger arrowed-in arrowed-in-right">${cxqqList.cljg_disp }</span>
                    </c:if>
                    </td>
