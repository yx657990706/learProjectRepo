	...
	<div class="page-content" id="page-content">
		<fm:form class="form-horizontal" commandName="cgb_risk_case"
			...
			<input type="hidden" id="re_url" name="re_url"
				value="<c:url value="/risk_case/list?isNewSearch=1"/>" />
			<!-- 右侧内容 -->
			<div class="row">
				<div class="col-xs-12">
					...
					<div class="widget-box">
						<div class="widget-header widget-header-flat">
							...
							<div class="widget-toolbar">
								<a href="#"
									onclick="to_open('<c:url value="/risk_case/add"/>','电信诈骗风险事件信息添加')">
									<i class="ace-icon fa fa-plus-circle orange"></i> 添加
								</a> &nbsp; <a href="#"
									onclick="do_tj('<c:url value="/risk_case/choose_save?bzw=1" />','确认提交审核?');">
									<i class="ace-icon fa fa-check green"></i> 提交审核
									...
									<tbody>
										<c:forEach items="${cgb_risk_caseList}" var="cases"
											...
											<tr class="text-center">
												<c:choose>
													<c:when test="${cases.status_cd=='0'}">
														...
														<td class="align-middle" nowrap="nowrap"><a
															class="green" href="#"
															onclick="to_open('<c:url value="/risk_case/modify?caseid=${cases.caseid}"/>','电信诈骗风险事件信息修改')"
															title="修改"> <i class="ace-icon fa fa-pencil "></i>
														</a> <a class="green" href="#"
															onclick="to_delete('<c:url value="/risk_case/delete?caseid=${cases.caseid}"/>','确认删除？')"
															title="删除"> <i class="ace-icon fa fa-trash-o red"></i>
														</a></td>
													</c:when>
													<c:otherwise>
														<td>--</td>
												...
												<td class="align-middle" nowrap="nowrap"><c:choose>
														<c:when test="${cases.status_cd=='0'}">
															<span
																class="label label-info arrowed-in arrowed-in-right">${cases.status_cd_disp}</span>
														</c:when>
														<c:when test="${cases.status_cd=='3'}">
															<span
																class="label label-danger arrowed-in arrowed-in-right">${cases.status_cd_disp}</span>
														</c:when>
