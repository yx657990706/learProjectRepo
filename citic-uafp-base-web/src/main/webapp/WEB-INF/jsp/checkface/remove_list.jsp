...
<body>
	<div class="page-content"  id="page-content">
		...
		<div class="row">
			<div class="col-xs-12">
				<fm:form id="myform" class="form-horizontal" commandName="cgb_face_data" 	action="remove_list" method="POST">
				<input type="hidden" id="re_url" name="re_url"
						value="<c:url value="/cgb_face_data/remove_list?isNewSearch=1"/>" />
									...
									<div class="form-group col-xs-12 col-sm-6">
										...
										<fm:select path="md_channel" class=" input-medium"
											...
											<button class="btn btn-info btn-mini" type="button"
												id="submitForm" onclick="_submit('remove_list?isNewSearch=1')">
			...
			<div class="widget-box widget-color-blue agg_widgetbox">
						...
						<table class="table table-striped table-bordered table-hover"
							...
							<tbody>
								<c:forEach items="${cgb_face_dataList}" var="removeList">
									<tr class="text-center">
										<td class="align-middle" nowrap><a href="#"
											onclick="_OpenXLarge('<c:url value="/cgb_face_data/group_query_list?group_id=${removeList.group_id}&bzw=3" />','查看本组信息')">
												${removeList. group_id}</a></td>
										<td class="align-middle" nowrap>${removeList.victim_name}</td>
										<td class="align-middle" nowrap>${removeList.victim_card_type_disp}</td>
										<td class="align-middle" nowrap>${removeList.victim_card_num}</td>
										<td class="align-middle" nowrap>${removeList.victim_card_validity}</td>
										<td class="align-middle" nowrap>${removeList.risk_type_disp}</td>
										<td class="align-middle" nowrap>${removeList.busi_website}-${removeList.busi_website_disp}</td>
										<td class="align-middle" nowrap>${removeList.busi_teller_num}</td>
										<td class="align-middle" nowrap>${removeList.busi_time}</td>
										<td class="align-middle" nowrap>${removeList.busi_type}</td>
										<td class="align-middle" nowrap>${removeList.busi_way_disp}</td>
										<td class="align-middle" nowrap>${removeList.telephone}</td>
										<td class="align-middle" nowrap>${removeList.address}</td>
										<td class="align-middle" nowrap>${removeList.md_channel_disp}</td>
										<td class="align-middle" nowrap>${removeList.check_user_id}</td>
										<td class="align-middle" nowrap>${removeList.check_result_disp}</td>
